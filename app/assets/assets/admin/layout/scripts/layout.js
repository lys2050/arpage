...
var Layout = function () {
    ...
    var handleSidebarMenu = function () {
        jQuery('.page-sidebar').on('click', 'li > a', function (e) {
            ...
            //    alert($(this).attr("href"))
                ...
                var opt = getParameterByName('opt', $(this).attr("href"));
                $.cookie('dpOptPage', opt);
        ...
        jQuery('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr, ajaxOptions, thrownError) {
                    Metronic.stopPageLoading();
                    pageContentBody.html('<h4>不能加载本次请求内容</h4>');
                }
            });
        });
    ...
    var handleHorizontalMenu = function () {
        //导航栏菜单点击后加载左侧菜单
        $('.page-header').on('click', '.hor-menu a[data-toggle="nav"]', function (e) {
            e.preventDefault();
            Metronic.scrollTop();

            var url = $(this).attr("href"),
              nav = $(".hor-menu .nav"),
              active_link = nav.find('li.current'),
              menuContent = $('.page-sidebar-menu');
            nav.find('li.active').removeClass("active");

            nav.find('.selected').remove();
           ...
           var mname= $.cookie('mname');
            ...
            $.ajax({
                ...
                dataType: "text",
                cache: false,
                url: "Data/MenuControl.ashx?_=" + (new Date()).getTime(),
                data: { "postPageID": 10101012, "ppid": id, "mname": mname },
                success: function (res) {

                    if (res) {
                      //  console.log('load left menu');
                       // menuContent.empty();
            ...
            $.ajax({
                ...
                error: function (xhr, ajaxOptions, thrownError) {
                    menuContent.html('<h4>不能加载本次请求内容</h4>');
                    Metronic.stopPageLoading();
                }
            });*/
        });
